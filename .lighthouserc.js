module.exports = {
  ci: {
    collect: {
      ...
      numberOfRuns: 3, // несколько прогонов для стабильности
    },
    assert: {
      // Пороговые значения для демонстрации (потом настроим жёстче)
      assertions: {
        "categories:performance": ["error", { minScore: 0.8 }], // общая оценка не ниже 80 баллов.
        "first-contentful-paint": ["error", { maxNumericValue: 2000 }], // FCP должен быть ≤ 2000 мс
        "largest-contentful-paint": ["error", { maxNumericValue: 30000 }], // LCP ≤ 30000 мс
        "cumulative-layout-shift": ["error", { maxNumericValue: 0.1 }], // CLS ≤ 0.1 (порог «хорошо»).
        "total-blocking-time": ["error", { maxNumericValue: 300 }], // TBT ≤ 300 мс.
      },
    },
    upload: {
